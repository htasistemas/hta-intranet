...
export class DashboardService {
  public async summary(userId: string) {
    ...
    const [total, active, inactive, todayAppointments, weekAppointments, pendingTasks, revenue, birthdays, categories, schedules, taskStatuses] = await prisma.$transaction([
    ...
    return {
      kpis: { total, active, inactive, todayAppointments, weekAppointments, pendingTasks, revenue: Number(revenue._sum.expectedValue ?? 0), birthdays },
