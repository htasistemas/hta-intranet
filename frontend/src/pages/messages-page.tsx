...
export default function MessagesPage() {
  ...
  const sortedTemplates = useMemo(() => [...(templates.data ?? [])].sort((left, right) => {
    const titleComparison = (left.subject ?? left.name).localeCompare(right.subject ?? right.name, "pt-BR");
    return titleComparison || left.name.localeCompare(right.name, "pt-BR");
  }), [templates.data]);
      ...
      <section className="grid gap-5 xl:grid-cols-[0.75fr_1.25fr]">
        ...
        <Card className="xl:col-span-2">
          ...
          <div className="space-y-2">
            {templates.isLoading ? <Skeleton className="h-32" /> : sortedTemplates.map((template) => (
              <div key={template.id} className="rounded-xl border border-slate-700 bg-sidebar p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="break-words font-medium">{template.subject ?? (template.channel === "WHATSAPP" ? "Mensagem para WhatsApp" : "Mensagem sem título")}</p>
                    <p className="mt-1 text-xs text-slate-400">Modelo: <span className="text-slate-200">{template.name}</span></p>
                    <p className="mt-1 text-xs text-slate-500">{template.channel} - {template.variables.join(", ") || "sem variáveis"}</p>
                  </div>
